...
{
  return m_map.vertexHandles().findIncidentBrushes(handle, selectedBrushes());
}

std::vector<mdl::BrushNode*> VertexTool::findIncidentBrushes(
  const vm::segment3d& handle) const
{
  return m_map.edgeHandles().findIncidentBrushes(handle, selectedBrushes());
}

std::vector<mdl::BrushNode*> VertexTool::findIncidentBrushes(
  const vm::polygon3d& handle) const
{
  return m_map.faceHandles().findIncidentBrushes(handle, selectedBrushes());
}
