...
void AdvancedEntitySelectDialog::onAccept()
{
  ...
  {
    predicate = [](tb::mdl::EntityNodeBase*) { return true; };
  }
  else if (m_brushEntities->isChecked())
  {
    predicate = [](tb::mdl::EntityNodeBase* entityNode) { return not entityNode->entity().pointEntity(); };
  }
  else
  {
    predicate = [](tb::mdl::EntityNodeBase* entityNode) { return entityNode->entity().pointEntity(); };
  }
  ...
  if (m_enableByClass->isChecked())
  {
    predicate = [this, oldPredicate = predicate](auto entityNode) {
      return oldPredicate(entityNode)
             and QString::fromStdString(entityNode->entity().classname()) == m_selectedClass->currentText();
    };
  }
