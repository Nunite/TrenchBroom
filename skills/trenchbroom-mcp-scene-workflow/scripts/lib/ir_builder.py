...
def _banked_arc_segment_points(
    center: list[float],
    inner_radius: float,
    outer_radius: float,
    start_angle: float,
    ...
    top = [
        polar_point(center, inner_radius, start_angle, z_inner),
        polar_point(center, outer_radius, start_angle, z_outer),
        polar_point(center, outer_radius, end_angle, z_outer),
        polar_point(center, inner_radius, end_angle, z_inner),
    ]
    bottom = [[point[0], point[1], point[2] - thickness] for point in top]
    return top + bottom


def op_banked_arc_segments(
    *,
    center: list[float],
    radius: float,
    width: float,
    ...
    z_inner = 0.0
    z_outer = math.tan(math.radians(cross_slope_degrees)) * width
    ...
    for index in range(segments):
        operation: dict[str, Any] = {
            "type": "polyhedron",
            "points": _banked_arc_segment_points(
                center,
                inner_radius,
                outer_radius,
