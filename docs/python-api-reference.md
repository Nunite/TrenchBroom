# TrenchBroom Python API 完整速查手册

本文档为 TrenchBroom 嵌入式 Python API (`trenchbroom`) 模块的完整 API 规格参考手册。
...
#### 方法
- `transaction(name: str = "Python API Script") -> Transaction` - 创建事务上下文管理器
- `save()` - 保存当前地图文档
- `reload()` - 从磁盘重新载入地图文档
...
### `trenchbroom.MaterialCollection`
...
- `trenchbroom.current_document() -> Document` - 获取当前激活的地图文档（若无打开地图则抛出异常）
- `trenchbroom.document() -> Document` - `current_document()` 的兼容别名
- `trenchbroom.selected_brushes()` / `trenchbroom.selectedBrushes() -> list[Brush]` - 获取直接选中的 Brush
- `trenchbroom.selected_entities(include_brushes=False)` / `trenchbroom.selectedEntities(...) -> list[Entity]` - 获取直接选中的实体；传入 `True` 时还包含选中 Brush 或单独选中面的父实体
- `trenchbroom.selected_all_entities()` / `trenchbroom.selectedAllEntities() -> list[Entity]` - 获取所有相关实体，语义与 `selection().all_entities` 相同
- `trenchbroom.selection() -> Selection` - 获取当前选区句柄
- `trenchbroom.selected_faces()` / `trenchbroom.selectedFaces() -> list[Face]` - 获取单独选中的 Brush 面
- `trenchbroom.translate(...)` / `trenchbroom.rotate(...)` / `trenchbroom.scale(...)` - 变换当前选区，或先将传入对象设为选区后变换
- `trenchbroom.duplicate(target=None)` - 复制当前选区或指定对象
- `trenchbroom.delete_selection()` / `trenchbroom.deleteSelection()` - 删除当前选区
- `trenchbroom.deselect_all()` / `trenchbroom.deselectAll()` - 清空当前选区
- `trenchbroom.create_plugin_panel(title: str) -> PluginPanel` - 在 Plugins 检查器中创建可视化面板
- `trenchbroom.create_brush(points: list[Vec3], material: str = None) -> Brush` - 根据凸包顶点集合生成 Brush
- `trenchbroom.list_actions() -> list[str]` - 列出当前所有已注册的编辑器 Action 标识符
- `trenchbroom.execute_action(action_id: str)` - 触发执行指定 Action
- `trenchbroom.register_callback(event: str, callback: Callable[[], None]) -> int` - 注册 `selection_changed`、`document_loaded` 或 `document_saved` 事件回调并返回 token
- `trenchbroom.unregister_callback(token: int)` - 注销事件回调
- `trenchbroom.set_timeout(callback: Callable[[], None], milliseconds: int) -> int` - 注册单次定时器
- `trenchbroom.set_interval(callback: Callable[[], None], milliseconds: int) -> int` - 注册循环定时器
- `trenchbroom.clear_interval(timer_id: int)` - 取消定时器
