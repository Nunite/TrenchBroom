...
拱形是作为空心圆柱体的上半部分创建的。轴向是拱形穿过的方向，类似于隧道。边数、厚度和圆形模式的工作方式与圆柱体完全相同，因此使用相同参数构建的拱形和圆柱体会完美贴合。当使用可缩放圆形模式时，将边界绘制得高于半圆会使两侧垂直向下延伸，作为拱形的“支撑柱”。勾选 _Spandrel（拱肩填充）_ 后，会使用额外的 Brush 填满拱形与包围盒之间的空隙。
