...
# 中文手册配置
set(PANDOC_TEMPLATE_CN_PATH "${DOC_MANUAL_SOURCE_DIR}/template.html")
set(PANDOC_INPUT_CN_PATH    "${DOC_MANUAL_SOURCE_DIR}/index_cn.md")
set(PANDOC_OUTPUT_CN_PATH   "${DOC_MANUAL_TARGET_DIR}/index_cn.html.tmp")
set(INDEX_CN_OUTPUT_PATH    "${DOC_MANUAL_TARGET_DIR}/index_cn.html")

fix_win32_path(PANDOC_TEMPLATE_PATH)
fix_win32_path(PANDOC_INPUT_PATH)
fix_win32_path(PANDOC_OUTPUT_PATH)
fix_win32_path(PANDOC_TEMPLATE_CN_PATH)
fix_win32_path(PANDOC_INPUT_CN_PATH)
fix_win32_path(PANDOC_OUTPUT_CN_PATH)
...
# 生成中文手册
add_custom_command(OUTPUT "${INDEX_CN_OUTPUT_PATH}"
    COMMAND ${CMAKE_COMMAND} -E make_directory "${DOC_MANUAL_TARGET_DIR}"
    COMMAND ${PANDOC_PATH} --standalone --toc --toc-depth=2 --template "${PANDOC_TEMPLATE_CN_PATH}" --from=markdown --to=html5 -o "${PANDOC_OUTPUT_CN_PATH}" "${PANDOC_INPUT_CN_PATH}"
    COMMAND ${CMAKE_COMMAND} -DINPUT="${PANDOC_OUTPUT_CN_PATH}" -DOUTPUT="${PANDOC_OUTPUT_CN_PATH}" -P "${CMAKE_CURRENT_BINARY_DIR}/AddVersionToManual.cmake"
    COMMAND ${CMAKE_COMMAND} -DINPUT="${PANDOC_OUTPUT_CN_PATH}" -DOUTPUT="${PANDOC_OUTPUT_CN_PATH}" -P "${CMAKE_CURRENT_SOURCE_DIR}/cmake/TransformKeyboardShortcuts.cmake"
    COMMAND ${CMAKE_COMMAND} -E copy "${PANDOC_OUTPUT_CN_PATH}" "${INDEX_CN_OUTPUT_PATH}"
    COMMAND ${CMAKE_COMMAND} -E remove "${PANDOC_OUTPUT_CN_PATH}"
    DEPENDS "${PANDOC_TEMPLATE_CN_PATH}" "${PANDOC_INPUT_CN_PATH}" "${CMAKE_CURRENT_SOURCE_DIR}/cmake/TransformKeyboardShortcuts.cmake" "${CMAKE_CURRENT_SOURCE_DIR}/cmake/AddVersionToManual.cmake.in"
)
...
add_custom_target(GenerateManual DEPENDS ${INDEX_OUTPUT_PATH} ${INDEX_CN_OUTPUT_PATH} ${DOC_MANUAL_TARGET_FILES_ABSOLUTE} ${DOC_MANUAL_SHORTCUTS_JS_TARGET_ABSOLUTE} ${DOC_MANUAL_TARGET_IMAGE_FILES_ABSOLUTE})
