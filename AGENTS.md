...
- Never use `ninja -t clean <object-file>` as a substitute for deleting one stale `.obj`. Ninja follows the target's dependency graph; an observed attempt to clean only `MapViewToolBox.cpp.obj` removed 1199 build outputs. For precise recovery, first resolve and verify the exact object path is inside `build-release-codex`, then delete only that file with a filesystem operation and rebuild the narrow target. If the environment blocks the exact deletion, stop and report it instead of falling back to Ninja clean or a clean wrapper.
